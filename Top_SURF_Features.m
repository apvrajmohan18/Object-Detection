...
close all
%%
[fname1,pname1]=uigetfile('*.jpg','Select Input Target Image');
u1a=imread(strcat(pname1,fname1));
figure,imshow(u1a)
title 'Input Image'
%%
[fname2,pname2]=uigetfile('*.jpg','Select Input Scene Image');
...
%% SURF
u3 = detectSURFFeatures(u1);
u4 = detectSURFFeatures(u2);
%% 
figure;
imshow(u1);
title('Feature Points from Target Image');
hold on;
plot(selectStrongest(u3, 100));

figure;
imshow(u2)
title('Feature Points from Target Image');
hold on;
plot(selectStrongest(u4, 100));
%% 
